...
from models.encoders.psp_encoders import NewEncoder
from models.extra import KpExtractor, KpEncoder
...
from criteria import id_loss, w_norm, moco_loss, kp_loss
from configs import transforms_config
from datasets.images_dataset import ImagesDataset, ImagesEditDataset
...
class ExtraTrainer:
    ...
    def __init__(self, opts):
        ...
        if checkpoint is not None:
            ...
            self.latent_avg = decoder_checkpoint['latent_avg']
        # self.face_pool = torch.nn.AdaptiveAvgPool2d((256, 256)).to(self.device)
        if self.latent_avg is None:
            self.latent_avg = self.decoder.mean_latent(int(1e5))[0].detach() if checkpoint is None else None
        self.encoder = Encoder(opts, checkpoint, self.latent_avg, device=self.device).to(self.device).eval()
        self.latent_avg = self.encoder.latent_avg
        if self.opts.new_inversion:
            self.new_encoder = NewEncoder(self.decoder).to(self.device)
            if "new_encoder" in checkpoint:
                self.new_encoder.load_state_dict(checkpoint["new_encoder"])
            else:
                logger.warning('New encoder state dict is not in checkpoint!')
        ...
        self.extra_feature_net = KpExtractor(device=self.device).eval()
        self.extra_net = KpEncoder(self.opts, g_layers=self.opts.g_layers).to(self.device)
        if checkpoint is not None and 'extra_net' in checkpoint:
            self.extra_net.load_state_dict(checkpoint['extra_net'])
    ...
    def configure_datasets(self):
        ...
        self.train_dataset = train_dataset = ImagesEditDataset(source_root=self.opts.train_dataset_path,
                                                               target_root=self.opts.train_dataset_path,
                                                               source_transform=transforms_dict['transform_edit'],
                                                               target_transform=transforms_dict['transform_edit'],
                                                               opts=self.opts)
        self.test_dataset = test_dataset = ImagesEditDataset(source_root=self.opts.test_dataset_path,
    ...
    def configure_loss(self):
        ...
        if self.opts.kp_lambda > 0:
            self.kp_loss = kp_loss.KPLossTrain(self.device).eval()

    def configure_optimizers(self, checkpoint):
        requires_grad(self.extra_net, True)
        requires_grad(self.extra_feature_net, False)
        requires_grad(self.encoder, False)
        requires_grad(self.decoder, False)
        betas = (self.opts.optim_beta1, self.opts.optim_beta2)
        optimizer = None
        if self.opts.new_inversion:
            if self.opts.new_optimizer == 'adam':
                self.new_optimizer = torch.optim.Adam(self.new_encoder.parameters(), lr=self.opts.new_lr,
                                                      weight_decay=self.opts.weight_decay, betas=betas)
            elif self.opts.new_optimizer == 'adamw':
                self.new_optimizer = torch.optim.AdamW(self.new_encoder.parameters(), lr=self.opts.new_lr,
                                                       weight_decay=self.opts.weight_decay, betas=betas)
            elif self.opts.new_optimizer == 'sgd':
                self.new_optimizer = torch.optim.SGD(self.new_encoder.parameters(), lr=self.opts.new_lr,
                                                     weight_decay=self.opts.weight_decay)
            else:
                self.new_optimizer = Ranger(self.new_encoder.parameters(), lr=self.opts.new_lr,
                                            weight_decay=self.opts.weight_decay, betas=betas)
            if checkpoint is not None:
                if 'new_optimizer' in checkpoint:
                    self.new_optimizer.load_state_dict(checkpoint['new_optimizer'])
                else:
                    logger.warning('New optimizer state dict is not in checkpoint!')
        else:
            if self.opts.optimizer == 'adam':
                optimizer = torch.optim.Adam(self.extra_net.parameters(), lr=self.opts.learning_rate,
                                             weight_decay=self.opts.weight_decay, betas=betas)
            elif self.opts.optimizer == 'adamw':
                optimizer = torch.optim.AdamW(self.extra_net.parameters(), lr=self.opts.learning_rate,
                                              weight_decay=self.opts.weight_decay, betas=betas)
            elif self.opts.optimizer == 'sgd':
                optimizer = torch.optim.SGD(self.extra_net.parameters(), lr=self.opts.learning_rate,
                                            weight_decay=self.opts.weight_decay)
            else:
                optimizer = Ranger(self.extra_net.parameters(), lr=self.opts.learning_rate,
                                   weight_decay=self.opts.weight_decay, betas=betas)
            if checkpoint is not None:
    ...
    def inverse(self, x, extra_info):
        with torch.no_grad():
            if self.opts.new_inversion:
                codes, feature_coarse, feature_middle, feature_fine = self.encoder(x)
                latents = self.new_encoder(codes.detach(),
                                           feature_coarse.detach(),
                                           feature_middle.detach(),
                                           feature_fine.detach())
            else:
        ...
        extra_latents = self.extra_net(extra_info[0].reshape(extra_info[0].shape[0], -1))
        codes[extra_info[1], 0:self.opts.g_layers, :] += extra_latents
        with torch.no_grad():
            images, result_latent = self.decoder([codes], input_is_latent=True, randomize_noise=True,
                                                 return_latents=True)
        # images = self.face_pool(images)
        return images, result_latent

    def train(self):
        self.extra_net.train()
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                # logger.info('step {}'.format(batch_idx))
                ...
                with torch.no_grad():
                    extra_info = self.extra_feature_net(y)
                if extra_info is None:
                    continue
                y_hat, latent = self.inverse(x, extra_info)
                loss, encoder_loss_dict, id_logs = self.calc_loss(x, y, y_hat, latent, extra_info)
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            with torch.no_grad():
                ...
                if extra_info is None:
                    continue
                y_hat, latent = self.inverse(x, extra_info)
                loss, cur_encoder_loss_dict, id_logs = self.calc_loss(x, y, y_hat, latent, extra_info)
            ...
            self.parse_and_log_images(id_logs, x, y, y_hat, title='images/test/faces',
                                      subscript='{:06d}'.format(batch_idx))
    ...
    def calc_loss(self, x, y, y_hat, latent, extra_info):
        loss_dict = {}
        loss = 0.0
        id_logs = None
        ...
        if self.opts.kp_lambda > 0:
            loss_kp = self.kp_loss(x, extra_info)
            loss_dict['loss_kp'] = float(loss_kp)
            loss += loss_kp * self.opts.kp_lambda
    ...
    def __get_save_dict(self, is_last):
        ...
        if self.opts.new_inversion:
            save_dict['new_encoder'] = self.new_encoder.state_dict()
        if self.opts.extra_weights:
            save_dict['extra_net'] = self.extra_net.state_dict()
        if is_last:
            if self.opts.new_inversion:
                save_dict['new_optimizer'] = self.new_optimizer.state_dict()
            else:

...
from torch.nn import Linear, Conv2d, BatchNorm2d, PReLU, Sequential, TransformerDecoderLayer
import torch.nn.functional as F
from enum import Enum

from models.encoders.helpers import get_blocks, Flatten, bottleneck_IR, bottleneck_IR_SE, _upsample_add
from models.stylegan2.model import EqualLinear, EqualConv2d, ScaledLeakyReLU, Generator
...
class NewBlock(Module):
    def __init__(self, in_channel: int, out_channel: int, stride: int, up_sample=False) -> None:
        super(NewBlock, self).__init__()
        self.transform = TransformerDecoderLayer(d_model=512, nhead=4, dim_feedforward=1024,
        ...
        self.scale = Sequential(
            ...
            Conv2d(in_channel, in_channel, kernel_size=3, stride=stride, padding=1, bias=False),
            PReLU(in_channel),
            Conv2d(in_channel, out_channel, kernel_size=3, stride=1, padding=1, bias=False),
            # PReLU(out_channel),
            BatchNorm2d(out_channel)
        )

    def forward(self, w: Tensor, feature_e: Tensor, feature_g: Tensor) -> Tensor:
        if self.up_sample:
            feature_g = F.interpolate(feature_g, scale_factor=self.stride, mode='bilinear', align_corners=True)
        feature_g_scale = self.scale(feature_g)
        feature_diff = feature_g_scale - feature_e
        feature_diff = feature_diff.flatten(2).permute(0, 2, 1)
        out = self.transform(w, feature_diff)
        return out


class NewEncoder(Module):
    def __init__(self, decoder, channel_multiplier=2) -> None:
        super(NewEncoder, self).__init__()
        self.channels = {
            4: 512,
            8: 512,
        ...
        '''self.transform_coarse = TransformerDecoderLayer(d_model=512, nhead=4, dim_feedforward=1024,
                                                        batch_first=True, norm_first=False)
        self.transform_middle = TransformerDecoderLayer(d_model=512, nhead=4, dim_feedforward=1024,
                                                        batch_first=True, norm_first=False)
        self.transform_fine = TransformerDecoderLayer(d_model=512, nhead=4, dim_feedforward=1024,
                                                      batch_first=True, norm_first=False)'''
        self.blocks = nn.ModuleList()
        for i in range(17):
            if i <= 2:
                out_channel, out_resolution = 512, 16
            elif i <= 6:
                out_channel, out_resolution = 512, 32
            else:
                out_channel, out_resolution = 512, 64

            in_resolution = int(2 ** ((i + 1) // 2 + 2))
            in_channel = self.channels[in_resolution]

            if out_resolution > in_resolution:
                up_sample = True
                stride = out_resolution // in_resolution
            else:
                up_sample = False
                stride = in_resolution // out_resolution
            self.blocks.append(NewBlock(in_channel, out_channel, stride, up_sample))

    def forward(self,
                latents: Tensor,
                feature_coarse: Tensor,
                feature_middle: Tensor,
                feature_fine: Tensor) -> Tensor:

        last_feature_map = None
        for i in range(17):
            if i <= 2:
                feature_e = feature_coarse
                # block = self.transform_coarse
            elif i <= 6:
                feature_e = feature_middle
                # block = self.transform_middle
            else:
                feature_e = feature_fine
                # block = self.transform_fine
            with torch.no_grad():
                last_feature_map = self.decoder.get_feature_map(latents[:, i], last_feature_map, i)
            # print(i, feature_e.size(), last_feature_map.size())
            delta = self.blocks[i](latents[:, i:i + 1], feature_e, last_feature_map)
            # delta = torch.randn([8, 1, 512]).cuda()
            latents[:, i + 1:i + 2] = latents[:, i + 1:i + 2] + delta     # .detach()
            print(i)
            time.sleep(10)
